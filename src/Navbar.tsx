...
export function Navbar(setter: pageProps) {
    ...
    if(user === null)
        mainContent = <button className="button is-white has-text-black ml-2"
                              onClick={() => setter.setPage("loginR")}>Login</button>;
    else
        mainContent = <button className="button py-1 is-white has-text-black ml-2"
                    ...
                    <div className="is-flex is-flex-direction-column is-align-items-center">
                        {user && (
                            <div className="box has-background-light px-2 py-0 mb-1 is-rounded is-size-7 is-flex is-align-items-center">
                                ...
                                <strong className="has-text-black mr-1">{user.username}</strong><br/>
                                {mainContent}
                            </div>
                        )}
                        {!user && mainContent}
